...
def eval(
    ...
    print(f"RUNNING ON TASKS {tasks}")
    ...
    for task in tasks:
        ...
        # if task not in task_files: # Always errors
        #     raise ValueError("Task %s not recognised!." % task)
    ...
    for task_id in range(num_tasks):
        ...
        task_num_episodes = 0
        task_num_successful_episodes = 0
        ...
        for ep in range(start_episode, start_episode + eval_episodes):
            ...
            try:
                ...
                print(f"Unknown except {e}")
                eval_env.shutdown()
                raise e

            print("INSPECTING TRANSITIONS IN ROLLOUT******************")
            for idx, transition in enumerate(episode_rollout):
                
                if np.isclose(getattr(transition, "reward"), 100) and getattr(transition, "terminal"):
                    # print(f"Great success at step {idx} in episode {ep} for task {task_id}")
                    task_num_successful_episodes += 1
            ...
            percentage = (task_num_successful_episodes/task_num_episodes)  * 100
            if verbose:
                print(
                    f"Evaluating {task_name} | Episode {ep} | Score: {reward} | Episode Length: {len(episode_rollout)} | Lang Goal: {lang_goal}"
                )
                print(f"Out of {task_num_episodes}, {task_num_successful_episodes} worked, or {percentage} %")
            eval_env.shutdown()
        ...
        if logging:
            ...
            with open(os.path.join(log_dir, task_name, csv_file), "a") as csv_fp:
                fieldnames = ["task", "task_num_episodes", "task_num_successful_episodes", "task_success_percentage"]
                csv_writer = csv.DictWriter(csv_fp, fieldnames=fieldnames)
                csv_results = {"task": task_name}
                csv_results["task_num_episodes"] = task_num_episodes
                csv_results["task_num_successful_episodes"] = task_num_successful_episodes
                csv_results["task_success_percentage"] = percentage
